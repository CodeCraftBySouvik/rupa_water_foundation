    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            <div class="card shadow-sm">
                ...
                <form method="POST" action="{{ route('inspection.update') }}" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="card-body">
                        ...
                        <div class="row g-3 mb-4">
                            <div class="col-4">
                                ...
                                <input type="date" name="checked_date"
                                    value="{{ old('checked_date', $inspection->checked_date) }}" class="form-control">
                            </div>
                            ...
                            <div class="col-4">
                                ...
                                <select name="checked_by" id="userSelect" class="form-select js-single-select-user">
                                    ...
                                    <option value="{{ $id }}" {{ old('checked_by', $inspection->checked_by) == $id ?
                                        'selected' : '' }}>
                                        {{ ucwords($name) }}
                                    </option>
                                    @endforeach
                            ...
                            <div class="col-4">
                                ...
                                <select name="location_id" id="locationSelect" class="form-select js-single-select">
                                    ...
                                    <option value="{{ $id }}" {{ old('location_id', $inspection->location_id) == $id ?
                                        'selected' : '' }}>
                                        {{ $title }}
                                    </option>
                                    @endforeach
                        ...
                        <div class="row row-cols-1 row-cols-md-4 g-3">
                            ...
                            <div class="mb-3">
                                ...
                                $inspection->tap_condition)) !!}
                                {!! pill('tap_condition','not present','Not Present', old('tap_condition',
                                $inspection->tap_condition)) !!}
                            ...
                            <div class="mb-3">
                                ...
                                $inspection->electric_meter_working)) !!}
                                {!! pill('electric_meter_working','no','No', old('electric_meter_working',
                                $inspection->electric_meter_working)) !!}
                                @error('electric_meter_working') <p class="text-danger small">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                $inspection->compressor_condition)) !!}
                            ...
                            <div class="mb-3">
                                ...
                                $inspection->electric_usage_method)) !!}
                                {!! pill('electric_usage_method','proper','Proper', old('electric_usage_method',
                                $inspection->electric_usage_method)) !!}
                                @error('electric_usage_method') <p class="text-danger small">{{ $message }}</p>
                                @enderror
                            </div>
                        </div>
                        ...
                        <div class="form-group mt-2">
                            ...
                            {{-- Existing Images --}}
                            @if($inspection->inspection_images()->count())
                            <div class="mt-2">
                                @foreach($inspection->inspection_images as $img)
                                <div style="display:inline-block; margin-right:10px; position:relative;">
                                    <img src="{{ asset($img->image_path) }}" alt="Image" width="80" height="80"
                                    ...
                                    {{-- <form id="delete-image-{{$img->id}}"
                                        action="{{ route('inspection.image.delete', $img->id) }}" method="POST"
                                        style="display:none;">
                                        @csrf
                                        @method('DELETE')
                                    </form> --}}
                            ...
                            @error('new_images') <p class="text-danger small">{{ $message }}</p> @enderror
                            @error('new_images.*') <p class="text-danger small">{{ $message }}</p> @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="notes" rows="3"
                                class="form-control">{{ old('notes', $inspection->notes) }}</textarea>
                        </div>
